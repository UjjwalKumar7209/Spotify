...
async function dynamicAlbum(folderName) {
  ...
  document.querySelector(".album").innerHTML += `
      <div id = "${folderName
        .replaceAll(" ", "_")
        .replaceAll(".", "_")}" class="songAlbum pointer container">
  ...
  document
    .querySelector(`#${folderName.replaceAll(" ", "_").replaceAll(".", "_")}`)
    ...
    if (element.href.endsWith(".mp3")) {
      ...
      document.querySelector(".songUL").innerHTML =
        ...
        `
            <li data-href = "${
              element.href
            }" class = "songNameAside pointer"><span class="leftMusic"><img src="music.svg" alt="">${element.href
          .split(`/${folderName.replaceAll(" ", "%20")}/`)[1]
          .replaceAll(
            "%20",
            " "
          )}</span><span>Play Now <img width = "18" src="play.svg" width="20" alt=""></span></li>
            `;
    }
  }
  playSong(folderName);
...
async function playSong(folderName) {
  document.querySelectorAll(".songNameAside").forEach((e) => {
    e.addEventListener("click", () => {
      ...
      setInterval(() => {
        document.querySelector(".songInfo").innerHTML = `<p>${currentSong.src
          .split(`/${folderName.replaceAll(" ", "%20")}/`)[1]
          .replaceAll("%20", " ")}</p>`;
        document.querySelector(".duration").innerHTML = `<p>${formatTime(
          currentSong.currentTime
        )} / ${formatTime(currentSong.duration)}</p>`;
